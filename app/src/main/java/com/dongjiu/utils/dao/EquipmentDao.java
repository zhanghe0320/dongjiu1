package com.dongjiu.utils.dao;

import android.content.Context;
import android.database.Cursor;


import com.dongjiu.utils.DBUtil.DBOpenHelper;
...
public class EquipmentDao {
  //  private SQLiteDatabase sqliteDatabase;// 创建SQLiteDatabase对象
    ...
    // 插入用户数据    使用中
    public synchronized void dbInsertEquipment( String equipmentname,String equipmentbase,String equipmenthost,String shelfState,String lackProduct) {
       // sqliteDatabase = dbOpenHelper.getWritableDatabase();// 以读写方法打开数据库，不仅仅是写，getReadableDatabase()是只读
        ...
        String sql = "insert into t_equipment (equipmentname,equipmentbase,equipmenthost,CreatedTime,shelfState,lackProduct)values (?,?,?,?,?,?)";
        // 传递过来的username与password分别按顺序替换上面sql语句的两个?，自动转换类型，下同，不再赘述
        Object bindArgs[] = new Object[] {equipmentname,equipmentbase.toLowerCase(),equipmenthost.toLowerCase(),time,shelfState,lackProduct};
        // 执行这条无返回值的sql语句
       ...
       // Log.i(TAG, "dbQueryOneByEquipmentname: 通过设备的名字查询设备！");
        ...
        try {
            ...
            if (cursor.moveToNext())// 判断Cursor中是否有数据
            {
                ...
                equipment.setLackProduct(cursor.getString(cursor.getColumnIndex("lackProduct")));
                equipment.setShelfState(cursor.getString(cursor.getColumnIndex("shelfState")));
    ...
    public String[] dbQueryAllEquipmengName() {
       ...
       // sqliteDatabase = dbOpenHelper.getWritableDatabase();
        String sql = "select equipmentname from t_equipment where equipmentname !='主设备'";
        Cursor cursor = null;
    ...
    // 删除设备    删除多余的数据
    public synchronized  void  dbDelete(String  equipment_host,String equipmentbase) {
     //   sqliteDatabase = dbOpenHelper.getWritableDatabase();
     ...
     //   Log.i(TAG, "dbDelete: 删除多余的数据");
       ...
       dbOpenHelper.getWritableDatabase().execSQL(sql, bindArgs);
      // int a =    dbOpenHelper.getWritableDatabase().delete("t_equipment", "equipmentbase=?", new String[]{equipmentbase});


       // Log.i(TAG, "dbDelete: "+a);

    }


    // 删除所有产品
    public void dbDeleteAllEquipment() {

        // sqliteDatabase = dbOpenHelper.getWritableDatabase();

        String sql = "delete from t_equipment where equipmentname != '主设备' ";


       // Log.i(TAG,"删除所有设备："+sql);

        Object bindArgs[] = new Object[]{GetName.getuserName().toLowerCase()};
    ...
    // 插入用户数据    使用中
    public synchronized void dbupdateEquipment( String equipmentname,String shelfState,String lackProduct,String equipmentbase) {
        // sqliteDatabase = dbOpenHelper.getWritableDatabase();// 以读写方法打开数据库，不仅仅是写，getReadableDatabase()是只读

       // Log.i(TAG, "dbupdate: 更新数据"+equipmentbase);
        String sql = "update  t_equipment set equipmentname=?,shelfState = ?,lackProduct=? where equipmentbase = ?";
        // 传递过来的username与password分别按顺序替换上面sql语句的两个?，自动转换类型，下同，不再赘述
        Object bindArgs[] = new Object[] {equipmentname,shelfState,lackProduct,equipmentbase.toLowerCase()};
        // 执行这条无返回值的sql语句
        //   Log.i(TAG, "dbInsertEquipment: 添加设备"+equipmentbase+equipmenthost+equipmentname);
        dbOpenHelper.getWritableDatabase().execSQL(sql, bindArgs);
    }
